...
- ✅ **第三方登录功能** - Google Sign-In（已实现），QQ（已实现），GitHub（预留）
...
### v1.0.14 (2026-06-01)
- ✅ **QQ 登录完整实现** - 通过 `tencent_kit` 插件实现 QQ 登录
- ✅ **QQ 权限授权修复** - 解决 QQ SDK 3.5.7+ "用户未授权"错误
- ✅ **QQ 用户信息获取** - 通过 QQ 开放接口补全 unionid、昵称、头像、性别、城市
- ✅ **QQ 返回数据统一** - 与 Google 登录结构兼容，前端类型同时支持
- ✅ **进度条通知点击修复** - 解决 Flutter Bridge 创建的进度条通知点击无响应问题
...
5. **QQ/TIM 设备兼容性**：
   - 设备上 QQ/TIM 版本过旧或损坏时，QQ SDK 调用失败
   - 需用户更新或重新安装客户端
...
| 版本 | 日期 | 主要更新 |
|------|------|----------|
| v1.0.14 | 2026-06-01 | QQ 登录完整实现、用户信息获取、进度条通知点击修复 |
